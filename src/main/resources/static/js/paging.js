var main = {
    init : function () {
        ...
        const totalPage = pageInfo.dataset.totalPages;
        const nowPage = pageInfo.dataset.offset;
        const limit = pageInfo.dataset.limit;
        // 하단에 보일 페이지 번호 수
        const maxPageView = 5;
        ...
        for(var i = 1; i <= pageView; i++) {
            ...
            $("#page_" + i).attr('href', '/products?offset=' + (pageButtonNum + i - 1) + '&limit=' + limit);
        }
        ...
        if (!isEndList) {
            pageDivTag.append($('<li class="page-item"><a class="page-link" href="/products?offset='+ pageDivision + '&limit=' + limit + '"><span>></span></a></li>'));
        }
        ...
        if (nowPage < maxPageView) {
            ...
            $("#prev").attr('href', '/products?offset='+ ((divisionCount - 1) * maxPageView - 1) + '&limit=' + limit );
        }
    }
};
